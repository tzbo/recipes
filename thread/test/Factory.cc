...
class StockFactory : boost::noncopyable {
   ...
   private:
    void deleteStock(Stock* stock) {
        ...
        if (stock) {
            muduo::MutexLockGuard lock(mutex_);
            stocks_.erase(stock->key());  // This is wrong, see removeStock
                                          // below for correct implementation.
        }
...
class StockFactory : public boost::enable_shared_from_this<StockFactory>,
   ...
   public:
    boost::shared_ptr<Stock> get(const string& key) {
        ...
        pStock = wkStock.lock();
        if (!pStock) {
            pStock.reset(new Stock(key), boost::bind(&StockFactory::deleteStock,
                                                     shared_from_this(), _1));
            wkStock = pStock;
        }
        return pStock;
