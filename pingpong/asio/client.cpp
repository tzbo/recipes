   ...
   public:
    ...
    void start(asio::ip::tcp::resolver::iterator endpoint_iterator) {
        ...
        socket_.async_connect(
            endpoint, strand_.wrap(boost::bind(&session::handle_connect, this,
   ...
   private:
    void handle_connect(const asio::error_code& err,
        ...
        if (!err) {
            ...
            asio::ip::tcp::endpoint endpoint = *endpoint_iterator;
            socket_.async_connect(
                endpoint, strand_.wrap(boost::bind(
...
class client {
   public:
    client(asio::io_service& ios,
           ...
           int timeout)
        : io_service_(ios), stop_timer_(ios), sessions_(), stats_() {
    ...
    try {
        ...
        while (--thread_count > 0) {
            asio::thread* new_thread =
                new asio::thread(boost::bind(&asio::io_service::run, &ios));
