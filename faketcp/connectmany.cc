...
void tcp_input(int fd,
    ...
    if (tcp_seg_len >= tcphdr_size && tcp_seg_len >= tcphdr->doff * 4) {
        ...
        printf("IP %s.%d > %s.%d: ", source, ntohs(tcphdr->source), dest,
               ntohs(tcphdr->dest));
        printf("Flags [%c], seq %u, win %d, length %d\n",
               tcphdr->syn ? 'S' : (tcphdr->fin ? 'F' : '.'),
               ntohl(tcphdr->seq), ntohs(tcphdr->window), payload_len);
        ...
        out.tcphdr.check =
            in_checksum(&out.iphdr.saddr, sizeof(struct tcphdr) + 12);
        if (response) {
...
bool connect_one(int fd, uint32_t daddr, int dport, uint32_t saddr, int sport) {
    ...
    if (nread >= iphdr_size && iphdr.version == 4 &&
        iphdr.ihl * 4 >= iphdr_size && iphdr.ihl * 4 <= nread &&
        iphdr.tot_len == htons(nread) && in_checksum(buf, iphdr.ihl * 4) == 0) {
...
int main(int argc, char* argv[]) {
    ...
    for (;;) {
        ...
        const int iphdr_len = iphdr.ihl * 4;
        if (nread >= iphdr_size && iphdr.version == 4 &&
            iphdr_len >= iphdr_size && iphdr_len <= nread &&
            iphdr.tot_len == htons(nread) && in_checksum(buf, iphdr_len) == 0) {
