...
struct uint128 {
    ...
    uint128(int32_t l) : low(l), high(l < 0 ? -1 : 0) {}

    uint128(uint64_t l = 0) : low(l), high(0) {}
...
// GCC needs to fix http://gcc.gnu.org/bugzilla/show_bug.cgi?id=51839
    ...
    static uint128 multiply128(uint64_t x,
                               uint64_t y)  // __attribute__ ((noinline))
    {
#if __x86_64__
        uint128 prod(x);
