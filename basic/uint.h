...
class UnsignedInt  // copyable
{
   public:
    ...
    bool lessThan(const UnsignedInt& x) const {
        ...
        return rhs.size() > limbs_.size() ||
               (rhs.size() == limbs_.size() &&
                std::lexicographical_compare(limbs_.rbegin(), limbs_.rend(),
                                             rhs.rbegin(), rhs.rend()));
    }
    ...
    void multiply(const UnsignedInt& x) {
        ...
        for (size_t i = 0; i < rhs.size(); ++i) {
            ...
            for (size_t j = 0; j < limbs_.size(); ++j) {
                uint64_t prod =
                    uint64_t(rhs[i]) * limbs_[j] + result[i + j] + carry;
    ...
    const value_type& getValue() const { return limbs_; }

    void setValue(int n, uint32_t x) { limbs_.assign(n, x); }

   private:
    void parseDec(const std::string& str);
    void parseHex(const std::string& str);

    uint32_t highest() const { return limbs_.empty() ? 0 : limbs_.back(); }
