...
class AsyncLoggingT : boost::noncopyable {
   public:
    ...
    void append(const char* logline, int len) { queue_.put(MSG(logline, len)); }
...
typedef AsyncLoggingT<string, muduo::BoundedBlockingQueue>
    AsyncLoggingBoundedQueue;
...
typedef AsyncLoggingT<LogMessage, muduo::BlockingQueue>
    AsyncLoggingUnboundedQueueL;
typedef AsyncLoggingT<LogMessage, muduo::BoundedBlockingQueue>
    AsyncLoggingBoundedQueueL;
