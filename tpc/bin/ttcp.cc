...
void transmit(const Options& opt) {
    ...
    if (stream->sendAll(&sessionMessage, sizeof(sessionMessage)) !=
...
void receive(const Options& opt) {
    ...
    struct SessionMessage sessionMessage = {0, 0};
    if (stream->receiveAll(&sessionMessage, sizeof(sessionMessage)) !=
        sizeof(sessionMessage)) {
    ...
    double total_mb =
        1.0 * sessionMessage.number * sessionMessage.length / 1024 / 1024;
    printf("%.3f MiB in total\n", total_mb);
    ...
    for (int i = 0; i < sessionMessage.number; ++i) {
        payload->length = 0;
        if (stream->receiveAll(&payload->length, sizeof(payload->length)) !=
        ...
        if (stream->receiveAll(payload->data, payload->length) !=
            payload->length) {
