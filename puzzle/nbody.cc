...
Planet bodies[5] = {Planet(Vector3(0, 0, 0), Vector3(0, 0, 0), solar_mass),
...
double energy() {
    ...
    for (Planet* planet1 = bodies; planet1 != bodies + number_of_bodies;
         ++planet1) {
        total_energy +=
            0.5 * planet1->mass * magnitude_squared(planet1->velocity);
        for (Planet* planet2 = planet1 + 1;
             planet2 != bodies + number_of_bodies; ++planet2) {
...
void offset_momentum() {
    ...
    for (Planet* planet = bodies + 2; planet != bodies + number_of_bodies;
         ++planet) {
