...
class Shuffler : boost::noncopyable {
   public:
    explicit Shuffler(int nbuckets) : buckets_(nbuckets) {
...
class Source {
   public:
    explicit Source(std::ifstream* in) : in_(in), count_(0), query_() {}
    ...
    bool operator<(const Source& rhs) const { return count_ < rhs.count_; }

    void output(std::ostream& out) { out << count_ << '\t' << query_ << '\n'; }
