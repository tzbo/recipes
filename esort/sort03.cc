...
struct Key {
    ...
    Key(const string& record, int idx) : index(idx) {
...
void sortWithKeys(const std::vector<string>& data, std::vector<Key>* keys) {
    ...
    // printf("make keys %f\n", data.size(), timeDifference(Timestamp::now(),
    // start));
...
class Task : public boost::enable_shared_from_this<Task> {
   public:
    Task(muduo::BlockingQueue<TaskPtr>* queue)
        : queue_(queue), id_(s_created++), sorted_(false) {
    ...
    bool read(InputFile& in) {
        ...
        printf("task %d read done   %s  %f  %zd \n", id_,
               readDone.toString().c_str(), timeDifference(readDone, startRead),
               data_.size());
        return !data_.empty();
    }
    ...
    void sort() {
        ...
        printf("task %d sort done   %s  %f\n", id_, sortDone.toString().c_str(),
               timeDifference(sortDone, startSort));
    ...
    void write(int batch) {
        ...
        printf("task %d write done  %s  %f\n", id_,
               writeDone.toString().c_str(),
               timeDifference(writeDone, startWrite));
    }
...
struct Record {
    ...
    Record(InputFile* in) : input(in) {}

    bool next() { return input->read(data, sizeof data) == kRecordSize; }
