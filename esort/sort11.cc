...
struct Record {
    ...
    Record(Source& src) : source(&src) { next(); }

    bool next() { return source->read(&value); }
...
class OutputFile : boost::noncopyable {
   public:
    ...
    void writeRecord(int64_t x) { fwrite_unlocked(&x, 1, sizeof x, file_); }

    void sync() { fdatasync(fileno(file_)); }
...
void output(muduo::BlockingQueue<BatchPtr>* queueOut,
    ...
    while (BatchPtr batch = queueOut->take()) {
        ...
        for (Batch::iterator it = batch->begin(); it != batch->end(); ++it) {
            out.writeRecord(*it);
        }
        out.sync();
...
int main(int argc, char* argv[]) {
...

    {
        ...
        for (std::vector<int64_t>::iterator it = data.begin(); it != data.end();
             ++it) {
            *it = lrand48();
        }
    }
