...
int main(int argc, char* argv[]) {
    ...
    // printf("cert parse %d\n", ret);
    x509_crt_parse(&cert, reinterpret_cast<const unsigned char*>(srv_cert),
                   strlen(srv_cert));
    x509_crt_parse(&cert, reinterpret_cast<const unsigned char*>(test_ca_list),
                   strlen(test_ca_list));

    pk_context pkey;
    pk_init(&pkey);
    pk_parse_key(&pkey, reinterpret_cast<const unsigned char*>(srv_key),
                 strlen(srv_key), NULL, 0);
    ...
    ecp_group_id curves[] = {POLARSSL_ECP_DP_SECP256R1,
                             POLARSSL_ECP_DP_SECP224K1, POLARSSL_ECP_DP_NONE};
    ssl_set_curves(&ssl_server, curves);
    if (useECDHE) {
        int ciphersuites[] = {TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA,
                              TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA, 0};
        ssl_set_ciphersuites(&ssl_server, ciphersuites);
    } else {
        int ciphersuites[] = {TLS_ECDH_ECDSA_WITH_AES_256_CBC_SHA,
    ...
    for (int i = 0; i < N; ++i) {
        ...
        while (true) {
            ...
            if (ret2 < 0) {
                ...
                // printf("server done %s %s\n", ssl_get_version(&ssl_server),
                // ssl_get_ciphersuite(&ssl_server));
            }
